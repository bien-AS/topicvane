...
import { ChartCard, CategoryBarChart } from '@/components/ui/chart-card'
...
import { getBacklinks, backlinksForTopic, topicCounts } from '@/lib/tokens/majestic-backlinks'
...
export function DomainWidgetsGallery() {
  ...
  return (
    <>
      <Section title="DomainCard (Projects grid)" subtitle="the S-21 project card — status · Trust Flow + trend · top categories · metrics · hover to lift">
      ...
      <Section title="TtfTopicRow — expandable" subtitle="Referring URLs & Links from On-Topic Domains expand to real backlinks (S-09). Majestic color on the value badge only">
        <Card className="p-2">
      ...
      <Section title="AnchorMatchRow" subtitle="anchor-phrase matches with relevancy — real anchor data from superiorgrouting.com">
        <Card className="p-4">
      ...
      <Section title="ChartCard & MetricSparkline" subtitle="restrained wrappers — a trend sparkline and a Majestic category bar chart">
        <div className="grid gap-5 md:grid-cols-2">
          <ChartCard title="Referring domains" unit="last 6 months" delta={{ value: '+37', direction: 'up' }}>
            <MetricSparkline data={[820, 870, 910, 980, 1010, 1049]} width={480} height={72} className="w-full" />
          ...
          <ChartCard title="Trust Flow by category" unit="foodchainid.com">
            <CategoryBarChart
              data={fc.topics.slice(0, 5).map((t) => {
                const child = t.topic.split(/\s*\/\s*/).slice(1).join(' / ')
                return {
      ...
      <Section title="LanguageBar" subtitle="share-of-links by language — neutral blue tints (languages aren't Majestic categories)">
