...
export function DataSurfacesGallery() {
  return (
    <>
      <Section
        title="ScoreRing"
        subtitle="Trust / Citation Flow · arc sweeps + counts up on load · score is neutral ink, ring is the single blue"
      >
        <div
          className="flex flex-wrap items-center gap-10 rounded-lg p-6"
          style={{
            background:
              "radial-gradient(circle at 50% -10%, hsl(var(--primary) / 0.08), transparent 60%)",
          }}
        >
          <ScoreRing
            value={fc.trustFlow}
            label="Trust Flow"
            sublabel="foodchainid.com"
          />
          <ScoreRing
      ...
      <Section
        title="StatCard"
        subtitle="label-caps + tabular metric (counts up) + delta + sparkline slot"
      >
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <StatCard
            label="Ref. IPs"
            value={fc.refIPs}
            icon={Icons.topicalMap}
          />
        </div>
      </Section>

      <Section
        title="TtfDistributionBar"
        subtitle="the signature share-of-Trust viz — segments grow in, Majestic colors, ranked legend"
      >
      ...
      <Section
        title="Table"
        subtitle="sticky header · hover rows · one emphasized column · right-aligned tabular numerics · blue links"
      >
      ...
      <Section
        ...
        subtitle="every data view ships both — never a dead end"
      >
      ...
      <Section
        title="Loading state"
        subtitle="table skeleton — pairs with the same Table layout above"
      >
        <div className="overflow-hidden rounded-lg border border-border">
          ...
          {[0, 1, 2, 3].map((r) => (
            <div
              key={r}
              className="flex items-center gap-4 border-b border-border/70 px-3 py-3 last:border-0"
            >
