...
const NAV_GROUPS: SidebarGroup[] = [
  ...
  { label: 'Account', footer: true, items: [{ key: 'account', label: 'Account', icon: Icons.account }] },
]
...
function AppFrameDemo() {
  ...
  return (
    <div className="h-[640px] overflow-hidden rounded-xl border border-border shadow-e2">
      <AppShell
        embedded
        sidebar={<Sidebar groups={NAV_GROUPS} activeKey={activeNav} onNavigate={setActiveNav} />}
        topBar={
          <TopBar
            domainContext={<DomainContextChip domain="foodchainid.com" mode={mode} onModeChange={setMode} />}
            action={
              <Button size="sm">
      ...
      >
        <div className="mx-auto max-w-[1100px] space-y-5">
          ...
          <div>
            <h1 className="font-heading text-[1.75rem] font-bold leading-tight tracking-tight">Domain Inventory</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              Expired domains you own, matched to topics by Trust Flow — {mode === 'fresh' ? 'fresh' : 'historic'} index.
            </p>
          </div>

          <div className="grid gap-4 sm:grid-cols-3">
            <StatCard label="Domains" value={DEMO_DOMAINS.length} icon={Icons.domainInventory} />
            <StatCard label="In use" value={2} icon={Icons.projects} />
            <StatCard label="Avg. Trust Flow" value={15} suffix="/ 100" icon={Icons.trustFlow} />
          </div>
...
export function AppShellGallery() {
  ...
  return (
    <>
      ...
      <Section title="Tabs" subtitle="underline workflow tabs — active = blue text + 2px underline">
        <Tabs defaultValue="topic">
          ...
          <TabsContent value="topic" className="text-[13px] text-muted-foreground">
            Pick the primary Topical Trust Flow category for this domain.
          </TabsContent>
          <TabsContent value="subtopics" className="text-[13px] text-muted-foreground">
            Choose up to 5 subtopics — each becomes a silo.
          </TabsContent>
          <TabsContent value="blog" className="text-[13px] text-muted-foreground">
            Set article types, quantity, and schedule.
          </TabsContent>
          <TabsContent value="site" className="text-[13px] text-muted-foreground">
            Choose a template and publish.
          </TabsContent>
        </Tabs>
      </Section>

      <Section title="SegmentedControl & DomainContextChip" subtitle="Fresh / Historic index toggle">
        <div className="flex flex-wrap items-center gap-6">
          <DomainContextChip domain="foodchainid.com" mode={mode} onModeChange={setMode} />
          <SegmentedControl
            options={[
