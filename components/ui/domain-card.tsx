...
export function DomainCard({
  ...
  const pct = progress && progress.total > 0 ? Math.round((progress.published / progress.total) * 100) : 0
  return (
    <Card
      interactive
      onClick={onOpen}
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="min-w-0">
            <div className="truncate font-heading text-[14px] font-semibold text-foreground">{domain}</div>
        ...
        <div className="flex shrink-0 flex-col items-end gap-1.5">
          ...
          {wordpress && (
            <span className="inline-flex items-center gap-1.5 text-[11px] font-medium text-muted-foreground" title={`WordPress ${wordpress}`}>
              <span
                className={cn('h-2 w-2 rounded-full', wordpress === 'connected' ? 'bg-success' : 'bg-destructive')}
                aria-hidden
              />
          ...
          <div className="min-w-0 flex-1 pl-4">
            <div className="flex items-baseline justify-between gap-2">
              ...
              <span className="text-[12px] tabular-nums text-muted-foreground">
                <span className="font-semibold text-foreground">{progress.published}</span> / {progress.total} live
              </span>
            </div>
            <Progress value={pct} className="mt-1.5 h-1.5" />
          </div>
        ) : (
          trend && trend.length > 1 && <MetricSparkline data={trend} width={104} height={34} />
        )}
      </div>
      ...
      {topTopics.length > 0 && (
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5">
          {topTopics.slice(0, 2).map((t) => (
            <span key={t.topic} className="inline-flex items-center gap-1.5">
              <TtfBadge topic={t.topic} value={t.value} />
      ...
      {metrics.length > 0 && (
        <div className="mt-auto grid grid-cols-2 gap-3 border-t border-border pt-4">
          {metrics.slice(0, 2).map((m) => (
            <BacklinkStat key={m.label} label={m.label} value={m.value} />
          ))}
        </div>
      )}
    </Card>
