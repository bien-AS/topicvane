  ...
  return (
    <div className={cn('w-full', className)} {...props}>
      <div className="mb-1 flex items-baseline justify-between">
        ...
        <span className="font-heading text-[15px] font-bold tabular-nums text-foreground">{valueFormat(max)}</span>
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" height={H} preserveAspectRatio="none" role="img" aria-label="Trend chart" className={cn(tone, 'overflow-visible')}>
        <defs>
        ...
        {grid.map((gy, i) => (
          <line key={i} x1="0" x2={W} y1={gy} y2={gy} className="stroke-border" strokeWidth={1} vectorEffect="non-scaling-stroke" />
        ))}
        ...
        <circle cx={last[0]} cy={last[1]} r={3.5} fill="currentColor" vectorEffect="non-scaling-stroke" />
      </svg>
