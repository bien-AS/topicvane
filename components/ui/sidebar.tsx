...
export function Sidebar({ groups, activeKey, onNavigate, sticky, className, ...props }: SidebarProps) {
  ...
  const renderItem = (item: SidebarItem) => {
    ...
    return (
      ...
      >
        ...
        <Icon className={cn('shrink-0', item.indent ? 'h-[16px] w-[16px]' : 'h-[18px] w-[18px]', active && 'text-primary')} />
        <span className="truncate">{item.label}</span>
        {item.badge && (
          <Badge variant="neutral" dot={false} className="ml-auto">
            {item.badge}
          </Badge>
        )}
      </button>
