...
export function LanguageBar({ segments, showLegend = true, height = 10, className, ...props }: LanguageBarProps) {
  const ranked = [...segments].sort((a, b) => b.value - a.value)
  const total = ranked.reduce((s, x) => s + x.value, 0) || 1

  return (
    <div className={cn('w-full', className)} {...props}>
      <div className="flex w-full overflow-hidden rounded-full bg-muted" style={{ height }} role="img" aria-label="Language distribution">
        {ranked.map((seg, i) => (
          <div
            key={seg.label}
        ...
        <ul className="mt-3 flex flex-wrap gap-x-5 gap-y-1.5">
          {ranked.map((seg, i) => (
            <li key={seg.label} className="flex items-center gap-2 text-[13px]">
              <span className="h-2.5 w-2.5 rounded-[3px]" style={{ backgroundColor: tintFor(i) }} aria-hidden />
