...
export interface TtfTopicRowProps {
  ...
  onTopicLinks: number
  backlinks?: Array<{ topicTf: number; sourceTf: number; source: string; target: string; anchor?: string }>
}
...
export function TtfTopicRow({ topic, value, referringUrls, referringDomains, onTopicLinks, backlinks = [] }: TtfTopicRowProps) {
  const [open, setOpen] = React.useState<Panel>(null)
  const { name } = getTtfColor(topic)
  const child = topic.split(/\s*\/\s*/).slice(1).join(' / ')
  ...
  return (
    <div className="border-b border-border last:border-0">
      <div className="flex items-center gap-3 px-3 py-2.5 text-[13px]">
        <TtfBadge topic={topic} value={value} />
        <span className="flex min-w-0 flex-1 items-baseline gap-2">
          <span className="font-medium text-foreground">{name}</span>
          {child && <span className="truncate text-muted-foreground">{child}</span>}
        </span>
        <button type="button" onClick={() => toggle('urls')} className={countBtn} aria-expanded={open === 'urls'}>
          {fmt(referringUrls)}
        </button>
        <span className="w-24 shrink-0 text-right tabular-nums text-muted-foreground">{fmt(referringDomains)}</span>
        <button type="button" onClick={() => toggle('ondomains')} className={countBtn} aria-expanded={open === 'ondomains'}>
          {fmt(onTopicLinks)}
        </button>
      </div>
                ...
                {backlinks.map((b, i) => (
                  <li key={i} className="flex items-center gap-3 px-3 py-2 text-[12px]">
                    <span className="flex w-12 shrink-0 items-center justify-center gap-1">
                      <span className="rounded-[4px] bg-muted px-1 font-heading text-[11px] font-bold tabular-nums text-foreground">{b.topicTf}</span>
                      {open === 'urls' && (
                        <span className="rounded-[4px] bg-accent px-1 font-heading text-[11px] font-bold tabular-nums text-accent-foreground">{b.sourceTf}</span>
                      )}
                    </span>
                    <span className="min-w-0 flex-1">
                      <a href="#" className="block truncate font-medium text-primary hover:underline">{short(b.source)}</a>
                      <span className="block truncate text-muted-foreground">↳ {short(b.target)}</span>
                    </span>
                    <span className="w-56 shrink-0 truncate text-muted-foreground">{b.anchor || '—'}</span>
                  </li>
                ))}
              </ul>
            </div>
          ) : (
            <p className="px-1 py-2 text-[12px] text-muted-foreground">No sample links for this topic.</p>
          )}
        </div>
      )}
    </div>
