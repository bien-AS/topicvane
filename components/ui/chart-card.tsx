...
export function ChartCard({ title, unit, delta, action, className, children, ...props }: ChartCardProps) {
  return (
...
export function CategoryBarChart({ data, className }: { data: CategoryBarChartDatum[]; className?: string }) {
  const max = Math.max(...data.map((d) => d.value), 1)
  return (
    <div className={cn('space-y-2.5', className)}>
      {data.map((d, i) => (
        <div key={`${d.label}-${i}`} className="flex items-center gap-3">
          <span className="w-28 shrink-0 truncate text-[13px] text-muted-foreground">{d.label}</span>
          <div className="h-4 flex-1 overflow-hidden rounded-[4px] bg-muted/60">
            <div className="h-full rounded-[4px]" style={{ width: `${(d.value / max) * 100}%`, backgroundColor: d.color }} />
          </div>
          <span className="w-7 shrink-0 text-right text-[13px] font-medium tabular-nums text-foreground">{d.value}</span>
        </div>
      ))}
    </div>
  )
